...
class Spreadsheet_Excel_Writer_Validator {
    ...
    const STYLE_STOP = 0x00;
    const STYLE_WARNING = 0x01;
    const STYLE_INFORMATION = 0x02;

    const TYPE_NONE = 0x00;
    const TYPE_WHOLE = 0x01;
    const TYPE_DECIMAL = 0x02;
    const TYPE_LIST = 0x03;
    const TYPE_DATE = 0x04;
    const TYPE_TEXTLENGTH = 0x05;
    const TYPE_CUSTOM = 0x06;
    ...
    function Spreadsheet_Excel_Writer_Validator($parser) {
        $this->_parser = $parser;
        $this->_type = 0x01; // : add method for setting datatype
        $this->_style = 0x00;
    ...
    function _getData($cellCoordinate) {
        ...
        try {
            $formula1 = $this->_formula1;
            if ($this->_type == 0x03) { // list type
