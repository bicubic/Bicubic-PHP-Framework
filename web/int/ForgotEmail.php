...
class ForgotEmail {
    ...
    function __construct(SystemUser $systemUser, Navigation $navigation) {
        $this->mandrillEmail = new MandrillEmail($systemUser->getEmail(), $navigation->config('web_contact_email'), $navigation->config('web_contact_name'), $navigation->lang('lang_emailforgotsubject'));
        $this->systemUser = $systemUser;
        $this->key = $navigation->config('mandrill_key');
        $this->template = $navigation->config('mandrill_template');
        $this->link = $navigation->application->getAppUrl("home","forgotValidate",array(new Param("token", $systemUser->getForgottoken())));
